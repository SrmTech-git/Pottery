...
/**
 * Get glazes by temperature (for compatibility checking)
 * @param {string} temperature - 'low' or 'high'
 * @returns {Array} Array of glazes matching the temperature
 */
export function getGlazesByTemperature(temperature) {
  const glazes = getActiveGlazes();
  return glazes.filter(glaze => glaze.temperature === temperature);
}
